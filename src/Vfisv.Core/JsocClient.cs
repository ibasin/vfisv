...
public sealed class JsocClient(HttpClient httpClient)
{
    ...
    public async Task<HmiObservationDescriptor> DescribeAsync(DateTimeOffset observationTime, CancellationToken cancellationToken = default)
    {
        ...
        var url = "http://jsoc.stanford.edu/cgi-bin/ajax/jsoc_info?ds=" + Uri.EscapeDataString($"hmi.S_720s[{record}]")
                  + "&op=rs_list&key=" + Uri.EscapeDataString(keys) + "&seg=" + Uri.EscapeDataString(string.Join(',', SegmentNames));
    ...
    public async Task DownloadSegmentsAsync(HmiObservationDescriptor observation, string directory,
