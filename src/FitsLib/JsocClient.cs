...
// Async JSOC record discovery/downloader replacing requests-based Python access
public sealed class JsocClient(HttpClient httpClient)
{
    private static readonly string[] SegmentNames = ["I0", "I1", "I2", "I3", "I4", "I5", "Q0", "Q1", "Q2", "Q3", "Q4", "Q5", "U0", "U1", "U2", "U3", "U4", "U5", "V0", "V1", "V2", "V3", "V4", "V5"];
    ...
    public async Task<HmiObservationDescriptor> DescribeAsync(DateTimeOffset observationTime, CancellationToken cancellationToken = default)
    {
        ...
        using var json = await JsonDocument.ParseAsync(await _httpClient.GetStreamAsync(url, cancellationToken), cancellationToken: cancellationToken);
